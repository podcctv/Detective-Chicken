...
const statCards = computed(() => [
  ...
  },
  {
    key: "media_degraded",
    label: "解锁异常",
    value: data.value.stats.media_degraded ?? 0,
    hint: "流媒体或 AI",
    icon: Cloud,
    tone: "violet",
  },
...
const mediaRows = computed(() => {
  ...
  if (media && Object.keys(media).length) {
    return Object.entries(media).map(([name, raw]) => ({
      name,
      status:
...
const scoreRows = computed(() =>
  Object.entries(selected.value?.latest_quality?.scores ?? {}).map(
    ([name, value]) => ({
      name,
...
const factorRows = computed(() =>
  Object.entries(selected.value?.latest_quality?.factors ?? {})
    .slice(0, 10)
    .map(([name, value]) => ({
      name,
...
<template>
  <div
    v-if="!auth || !auth.settings.bootstrapped || resetToken"
    class="auth-shell"
  >
    <div class="auth-brand">
      <div class="brand-mark">探</div>
      <div><strong>鸡探长</strong><span>DETECTIVE CHICKEN</span></div>
    </div>
    <section class="auth-panel" aria-labelledby="auth-title">
      <template v-if="resetToken">
        ...
        <form @submit.prevent="completeReset">
          ...
          <button class="primary-btn auth-submit" :disabled="authBusy">
            确认重置
          </button>
        </form>
      </template>
      <template v-else>
        <div class="auth-heading">
          ...
          <div>
            <h1 id="auth-title">
              {{
                auth?.settings.bootstrapped ? "登录控制台" : "创建首位管理员"
              }}
            </h1>
            <p>
              {{
                auth?.settings.bootstrapped
                  ? "管理 VPS IP 质量、解锁能力与 Agent。"
                  : "第一个注册账户将自动获得管理员权限。"
              }}
            </p>
          </div>
        ...
        <div
          v-if="
            auth?.settings.bootstrapped && auth.settings.registration_enabled
          "
          class="segmented"
        >
          <button
        ...
        <form @submit.prevent="submitAuth">
          ...
          /></label>
          <p v-if="authError" class="form-error">{{ authError }}</p>
          <p v-if="fatalError" class="form-error">{{ fatalError }}</p>
          <button class="primary-btn auth-submit" :disabled="authBusy">
        ...
        <p
          ...
          class="auth-footnote"
        >
          当前实例已关闭新用户注册，请联系管理员开放。
        </p>
      </template>
    </section>
  </div>

  <div v-else-if="!auth.authenticated">
    <PublicShowcase
      :data="data"
      :loading="loading"
      :dark="dark"
      ...
      >
        ...
        >
          <div class="modal-head">
            <div>
              <h2 id="public-login-title">登录控制台</h2>
              <p>登录后管理完整 IP、Agent、告警和检测周期</p>
            </div>
            <button
              class="icon-btn"
              aria-label="关闭登录"
          ...
          <div class="public-auth-body">
            <div v-if="auth.settings.registration_enabled" class="segmented">
              <button
                :class="{ active: authMode === 'login' }"
                @click="authMode = 'login'"
              >
            ...
            <p v-if="!auth.settings.registration_enabled" class="auth-footnote">
              当前实例已关闭新用户注册。
            </p>
          </div>
        </section>
  ...
  <div v-else class="app-shell">
    ...
    <aside class="sidebar" :class="{ open: menuOpen }">
      ...
      <nav aria-label="主导航">
        <a class="nav-item active" href="#dashboard"
          ><LayoutDashboard :size="18" /><span>质量总览</span></a
        ><a class="nav-item" href="#fleet"
          ><Server :size="18" /><span>节点资产</span
          ><small>{{ data.stats.total ?? 0 }}</small></a
        ><a class="nav-item" href="#alerts"
          ><Bell :size="18" /><span>告警中心</span
          ><small class="danger-count">{{ data.alerts.length }}</small></a
        >
      </nav>
      <div class="sidebar-section">账户与系统</div>
      <nav aria-label="管理导航">
        <button class="nav-item nav-button" @click="passwordOpen = true">
          <KeyRound :size="18" /><span>修改密码</span></button
        ><button
          v-if="auth.user?.role === 'admin'"
          class="nav-item nav-button"
          @click="openAdmin"
        >
          <UserCog :size="18" /><span>用户管理</span></button
        ><button class="nav-item nav-button" @click="logout">
    ...
    <main id="main-content" class="main-content">
      <header class="topbar">
        <div class="page-heading">
          ...
          <div>
            <h1>小鸡质量总览</h1>
            <p>优先查看 IP 质量、流媒体与 AI 解锁状态</p>
          </div>
        </div>
        <div class="top-actions">
          <span class="env-badge"><i></i>实时数据</span
          ><button class="icon-btn" title="切换主题" @click="toggleDark">
            <Sun v-if="dark" :size="18" /><Moon v-else :size="18" /></button
          ><button class="icon-btn" title="刷新" @click="loadDashboard(true)">
            <RefreshCw :size="18" :class="{ spinning: refreshing }" /></button
      ...
      <div v-if="fatalError" class="page-error">
        <AlertTriangle :size="17" />{{ fatalError }}
      </div>

      <section id="dashboard" class="kpi-grid" aria-label="关键指标">
        <article
        ...
        >
          <div class="kpi-icon"><component :is="card.icon" :size="18" /></div>
          <div>
      ...
      <section class="quality-grid">
        <div class="panel ranking-panel">
          <div class="panel-head">
            <div>
              <h2>小鸡质量排行榜</h2>
              <p>低风险优先，同分时按解锁数量排序</p>
            </div>
            <CircleGauge :size="19" />
          </div>
          <div v-if="data.rankings.length" class="ranking-list">
            <button
              v-for="item in data.rankings.slice(0, 8)"
              :key="item.node_id"
              @click="openNode(data.nodes.find((n) => n.id === item.node_id)!)"
            >
              ...
              ><span class="ranking-name"
                ...
                ><small>{{ item.provider }} · {{ item.region }}</small></span
              ><span class="unlock-count"
                >{{ item.unlocks }}/2<small>解锁</small></span
              ><strong :class="riskClass(item.risk)">{{ item.quality }}</strong
              ><ChevronRight :size="16" />
            </button>
          </div>
          <div v-else class="empty-state compact">
            <Server :size="22" /><strong>还没有节点</strong
            ><span>添加第一台 VPS 后生成质量排名</span>
          </div>
        </div>
        <div class="panel unlock-panel">
          <div class="panel-head">
            <div>
              <h2>解锁能力矩阵</h2>
              <p>快速定位流媒体与 AI 服务限制</p>
            </div>
            <Cloud :size="19" />
          </div>
          <div v-if="data.nodes.length" class="unlock-list">
            <button
              v-for="node in data.nodes.slice(0, 8)"
              :key="node.id"
              @click="openNode(node)"
            >
              <span
                ><strong>{{ node.name }}</strong
                ><small>{{ node.masked_ip }}</small></span
              ><StatusBadge :value="node.netflix" kind="media" /><StatusBadge
                :value="node.chatgpt"
                kind="media"
              /><ChevronRight :size="16" />
            </button>
          ...
          <div v-else class="empty-state compact">
            <Cloud :size="22" /><strong>暂无解锁数据</strong
            ><span>首次扫描完成后在这里展示</span>
          </div>
        </div>
      </section>
      ...
      <section class="analytics-grid">
        <div class="panel trend-panel">
          <div class="panel-head">
            <div>
              <h2>风险与信誉趋势</h2>
              <p>{{ data.nodes[0]?.name ?? "等待节点" }} · 最近采样</p>
            </div>
            <span class="trend-change"
              >{{ data.nodes[0]?.risk ?? 0 }}<small>当前风险</small></span
            >
          </div>
          <TrendChart :points="data.trend" />
          <div class="chart-foot">
            <span><i class="anomaly-dot"></i>综合风险 / IPQS / Scamalytics</span
        ...
        <div id="alerts" class="panel alert-panel">
          ...
          <div v-if="data.alerts.length" class="alert-list">
            <button
              v-for="alert in data.alerts.slice(0, 4)"
              :key="alert.id"
              class="alert-row"
              @click="openNode(data.nodes.find((n) => n.id === alert.node_id)!)"
            >
              <span class="alert-mark" :class="alert.severity"
                ><ShieldAlert
                  v-if="alert.severity === 'critical'"
                  :size="16" /><AlertTriangle v-else :size="16" /></span
              ><span class="alert-copy"
                ><strong>{{ alert.title }}</strong
                ><span>{{ alert.node_name }} · {{ alert.detail }}</span
                ><small>{{ relative(alert.created_at) }}</small></span
              ><ChevronRight :size="16" />
            </button>
          </div>
          <div v-else class="empty-state compact">
            <Activity :size="22" /><strong>当前没有告警</strong
            ><span>节点状态保持稳定</span>
          </div>
        </div>
      </section>

      <section id="fleet" class="panel fleet-panel">
        <div class="fleet-head">
          <div>
            <h2>节点资产</h2>
            <p>{{ filteredNodes.length }} 个结果，默认隐藏 IP 后两段</p>
          </div>
          <div class="fleet-tools">
            <label class="search-field"
              ><Search :size="16" /><input
                v-model="search"
                type="search"
                placeholder="搜索节点、地区或 ASN" /></label
            ><label class="filter-field"
              ><ListFilter :size="16" /><select v-model="filter">
        ...
        <div class="table-wrap">
              ...
              >
                <td>
                  <div class="node-name">
                    ...
                    <div>
                      <strong>{{ node.name }}</strong
                      ><small>{{ node.provider || "未标记服务商" }}</small>
                    </div>
                  </div>
                </td>
                <td>
                  <code>{{ node.masked_ip }}</code
                  ><small class="family">IPv{{ node.family || 4 }}</small>
                </td>
                <td>
                  <strong class="asn">AS{{ node.asn || "—" }}</strong
                  ><small>{{ node.region || node.organization }}</small>
                </td>
                <td>
                  <div class="risk-cell">
                    <strong :class="riskClass(node.risk)">{{
                      node.risk
                    }}</strong
                    ><span>{{ riskLabel(node.risk) }}风险</span>
                  </div>
                </td>
                <td><StatusBadge :value="node.netflix" kind="media" /></td>
                <td><StatusBadge :value="node.chatgpt" kind="media" /></td>
                <td>
                  <span class="dnsbl" :class="{ hit: node.dnsbl > 0 }">{{
                    node.dnsbl
                  }}</span>
                </td>
                <td>
                  <span class="last-seen">{{ relative(node.last_seen) }}</span>
                </td>
          ...
          <div v-if="filteredNodes.length === 0" class="empty-state">
            <Search :size="24" /><strong>{{
              data.nodes.length ? "没有匹配的节点" : "还没有纳管 VPS"
            }}</strong
            ><span>{{
              data.nodes.length
                ? "调整搜索词或状态筛选"
                : "点击右上角添加 VPS 生成兼容安装脚本"
            }}</span>
          </div>
        </div>
      </section>
      <footer>
        <span>鸡探长 · 数据按账户隔离，IP 默认隐藏后两段</span
        ><span>心跳 2 min · 质量检测周期按节点配置</span>
      </footer>
    </main>
    ...
    <Transition name="drawer"
      ...
      >
        <aside class="detail-drawer wide" aria-label="节点详情">
          <div v-if="detailLoading && !selected" class="drawer-loading">
            正在读取节点报告…
          </div>
          <template v-if="selected"
            ><div class="drawer-head">
              <div>
                ...
                <div>
                  <h2>{{ selected.name }}</h2>
                  <p>{{ selected.provider }} · {{ selected.region }}</p>
                </div>
              </div>
              <button
                class="icon-btn"
            ...
            <div class="drawer-summary">
              <div>
                <span>综合风险</span
                ><strong :class="riskClass(selected.risk)">{{
                  selected.risk
                }}</strong>
              </div>
              <div>
                <span>当前状态</span><StatusBadge :value="selected.status" />
              </div>
              <div>
                <span>质量评分</span><strong>{{ 100 - selected.risk }}</strong>
              </div>
            </div>
            <div class="detail-section">
              <div class="section-title">
                ...
                <button
                  ...
                  @click="toggleFullIP"
                >
                  <EyeOff v-if="showFullIP" :size="15" /><Eye
                    v-else
                    :size="15"
                  />{{ showFullIP ? "隐藏完整 IP" : "显示完整 IP" }}
                </button>
              </div>
              <dl>
                <div>
                  <dt>公网 IP</dt>
                  <dd>
                    <code>{{ selected.ip_address || selected.masked_ip }}</code>
                  </dd>
                </div>
                <div>
                  <dt>ASN</dt>
                  <dd>
                    AS{{ selected.asn || "—" }} ·
                    {{ selected.organization || "等待检测" }}
                  </dd>
                </div>
                <div>
                  <dt>网络族</dt>
                  <dd>IPv{{ selected.family || 4 }}</dd>
                </div>
                <div>
                  <dt>上次扫描</dt>
                  <dd>{{ relative(selected.last_scan) }}</dd>
                </div>
              </dl>
              <div v-if="selected.networks?.length" class="network-stack">
                <div v-for="network in selected.networks" :key="network.family">
                  <span>
                    <strong>IPv{{ network.family }}</strong>
                    <small
                      >{{ network.country_code || "--" }} · AS{{
                        network.asn || "—"
                      }}</small
                    >
                  </span>
                  <code>{{ network.ip_address || network.masked_ip }}</code>
                  <span>
                    <strong :class="riskClass(network.risk)">{{
                      100 - network.risk
                    }}</strong>
                    <small>质量分</small>
                  </span>
            ...
            <div class="detail-section schedule-section">
              ...
              <p
                v-if="selected.quality_status === 'scanning'"
                class="scan-state scanning"
              >
                IPv4 / IPv6 首次质量检测进行中，通常需要 1–3 分钟。
              </p>
              <p
                v-else-if="
                  selected.quality_status === 'failed' ||
                  selected.quality_status === 'partial'
                "
                class="scan-state failed"
              >
                {{
                  selected.last_scan_error ||
                  "部分网络检测失败，请查看 Agent 日志。"
                }}
              </p>
            </div>
            <div class="detail-section">
              ...
              <div class="media-detail-grid">
                <div v-for="media in mediaRows" :key="media.name">
                  <span>{{ media.name }}</span
                  ><StatusBadge :value="media.status" kind="media" />
                </div>
              </div>
            </div>
            <div v-if="scoreRows.length" class="detail-section">
              <h3>IP 质量评分</h3>
              <div class="score-grid">
                <div v-for="score in scoreRows" :key="score.name">
                  <span>{{ score.name }}</span
                  ><strong :class="riskClass(score.value)">{{
                    score.value
                  }}</strong
                  ><i
                    ><b :style="{ width: `${Math.min(100, score.value)}%` }"></b
                  ></i>
                </div>
              </div>
            </div>
            <div v-if="factorRows.length" class="detail-section">
              <h3>风险因子</h3>
              <dl>
                <div v-for="factor in factorRows" :key="factor.name">
                  <dt>{{ factor.name }}</dt>
                  <dd>{{ factor.value }}</dd>
                </div>
              </dl>
            ...
            <div class="detail-section">
              <h3>风险趋势</h3>
              <TrendChart :points="selected.series" />
            </div>
            <div class="detail-section">
              <h3>最近变化与告警</h3>
              <div v-if="selected.alerts.length" class="change-list">
                <div v-for="alert in selected.alerts" :key="alert.id">
                  <AlertTriangle :size="17" /><span
                    ><strong>{{ alert.title }}</strong
                    ><small
                      >{{ alert.detail }} ·
                      {{ relative(alert.created_at) }}</small
                    ></span
                  >
                </div>
              </div>
              <div v-else class="quiet-state">
                <Activity :size="17" />未发现显著变化
              </div>
            </div>
            <div class="drawer-actions">
              <button class="secondary-btn" @click="selected = null">
                关闭</button
              ><button class="primary-btn" @click="scan(selected)">
                <ScanLine :size="17" />立即扫描
              </button>
            </div></template
          >
        </aside>
    ...
    <Transition name="modal"
      ...
      >
        <section class="modal large-modal" role="dialog" aria-modal="true">
          <div class="modal-head">
            <div>
              <h2>添加 VPS</h2>
              <p>根据目标系统、架构与运行环境生成安装脚本</p>
            </div>
            <button class="icon-btn" @click="closeEnrollment">
              <X :size="19" />
            </button>
          </div>
          <form v-if="!enrollment" @submit.prevent="createEnrollment">
            <label
              >节点名称<input
                ...
                required
                placeholder="例如 HK-CMI-01"
            /></label>
            <div class="form-grid">
              <label
                >服务商<input
                  v-model="enrollForm.provider"
                  placeholder="例如 OVH" /></label
              ><label
                >地区标签<input
                  v-model="enrollForm.region"
                  placeholder="例如 加拿大"
              /></label>
            </div>
            <div class="form-grid three">
              <label
                >系统<select v-model="enrollForm.os_family">
                  <option value="auto">自动识别</option>
                  <option value="debian">Debian / Ubuntu</option>
                  <option value="alpine">Alpine</option>
                  <option value="rhel">RHEL / Rocky</option>
                  <option value="arch">Arch Linux</option>
                </select></label
              ><label
                >环境<select v-model="enrollForm.platform">
                  <option value="auto">自动识别</option>
                  <option value="pve">PVE 宿主机</option>
                  <option value="baremetal">独立服务器</option>
              ...
              ><label
                >架构<select v-model="enrollForm.arch">
                  <option value="auto">自动识别</option>
            ...
            <label
              >质量检测周期<select v-model="enrollForm.scan_interval_minutes">
                <option :value="60">每 1 小时</option>
            ...
            >
            <div class="form-note">
              <ShieldAlert :size="17" /><span
                >凭证 10 分钟内有效且只能使用一次。安装后会立即并行检测可用的
                IPv4 / IPv6，通常 1–3 分钟；周期也可在节点详情中随时调整。</span
              >
            </div>
          ...
          <div v-else class="enrollment-result">
            <div class="step-status">
              <span>1</span>
              <div>
                <strong>兼容安装脚本已生成</strong
                ><small
                  >{{
                    new Date(enrollment.expires_at).toLocaleTimeString("zh-CN")
                  }}
                  前有效</small
                >
              </div>
            </div>
            <label>在目标 VPS 上执行</label>
            <div class="command-box">
              <code>{{ enrollment.install_command }}</code
              ><button
                class="icon-btn"
                title="复制"
                @click="copyText(enrollment.install_command, '安装命令已复制')"
              >
                <Copy :size="17" />
              </button>
            </div>
            <div class="installation-steps">
              <div class="done"><i></i>环境检测</div>
              <div><i></i>Agent 注册</div>
              <div><i></i>心跳启动</div>
              <div><i></i>首次检测</div>
            </div>
            <div class="modal-actions">
              <button class="secondary-btn" @click="enrollment = null">
                返回</button
              ><button class="primary-btn" @click="closeEnrollment">
                完成
              </button>
            </div>
          </div>
      ...
      >
        <section class="modal admin-modal">
          <div class="modal-head">
            <div>
              <h2>用户与权限</h2>
              <p>管理注册开关、账户角色和密码重置</p>
            </div>
            <button class="icon-btn" @click="adminOpen = false">
              <X :size="19" />
            </button>
          </div>
          <div class="admin-settings">
            <label class="toggle-row"
              ><span
                ...
                ><small>开启后，新注册账户默认为普通用户</small></span
              ><input
          ...
          <div class="user-table">
            <div v-for="user in users" :key="user.id" class="user-row">
              ...
              ><span
                ...
                ><small
                  >@{{ user.username }} · {{ relative(user.created_at) }}</small
                ></span
              ><select
                :value="user.role"
                :disabled="user.id === auth.user?.id"
                @change="
                  updateRole(user, ($event.target as HTMLSelectElement).value)
                "
              >
              ...
              ><button
                class="icon-btn"
                title="生成密码重置链接"
                @click="createReset(user)"
              >
                <KeyRound :size="16" />
              </button>
            </div>
          </div>
          <div v-if="resetLink" class="reset-result">
            <span>密码重置链接（30 分钟有效）</span><code>{{ resetLink }}</code
            ><button
              class="secondary-btn"
              @click="copyText(resetLink, '重置链接已复制')"
            >
              <Copy :size="15" />复制
            </button>
          </div>
      ...
      >
        <section class="modal">
          <div class="modal-head">
            ...
            <button class="icon-btn" @click="passwordOpen = false">
              <X :size="19" />
            </button>
          </div>
          <form @submit.prevent="changePassword">
            ...
            <div class="modal-actions">
              ...
              >
                取消</button
              ><button class="primary-btn">更新密码</button>
            </div>
          </form>
        </section>
      </div></Transition
